...
Runs the synchronous Gemini SDK call in a thread executor so it never blocks
...
async def gemini_call_with_retry(fn, *args, **kwargs):
    """
    Call a synchronous Gemini SDK function in a thread executor with
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            # Run the blocking SDK call in a thread pool so the event loop stays free
